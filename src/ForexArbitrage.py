...
class Graph:
    ...
    def build_graph(self):  # Building a matrix data structure from yahoo and appending edges to graph
        ...
        for i in range(self.vertices):
            for j in range(self.vertices):
                if i == j:
                    ...
                    aux = str(self.tickers[i] + self.tickers[j] + '=X')     # Concatenating --> ex. "USDEUR=X"
                    data = round(pd.get_data_yahoo(aux).iat[-1, -1], 5)     # Live exchange rate in adj close column

                self.currency_matrix[i][j] = data                           # Keeping track of exchange rates
    ...
    def Bellman_Ford(self):
        ...
        neg_cycle = self.Negative_Cycle(dist, path)
    ...
    def Negative_Cycle(self, dist, path):
        ...
        for s, e, v in self.arches:
            if dist[s] + v < dist[e] and dist[s] != float("Inf"):     # Verifying distance after the algo has converged
                neg_cycle = [e, s]
                ...
                if neg_cycle[0] == neg_cycle[-1] and len(neg_cycle) > 3:  # Selecting valid cycle
                    return neg_cycle
    ...
    def Display_Graph(self, path, profit, flag):
        ...
        if flag == 0:
            ...
            for i in range(len(path)-1):                        # Colouring the negative cycle
